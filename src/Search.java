...
public class Search {
     ...
     * the solver string like in F' R B R L2 F .
     * U2 U D for example.<br>
     */
    public static final int USE_SEPARATOR = 0x1;

    /**
     * Verbose_Mask determines if the solution will be inversed to a scramble/state
     * generator.
     */
    public static final int INVERSE_SOLUTION = 0x2;

    /**
     * Verbose_Mask determines if a tag such as "(21f)" will be appended to the
     * solution.
     */
     ...
     *                 </pre>
     * 
     *                 A cube definition string "UBL..." means for example: In
     *                 position U1 we have the U-color, in position U2 we have the
     *                 B-color, in position U3 we have the L color etc. For example,
     *                 the "super flip" state is represented as <br>
     * 
     *                 <pre>
     *                 UBULURUFURURFRBRDRFUFLFRFDFDFDLDRDBDLULBLFLDLBUBRBLBDB
     *                 </pre>
     * 
     *                 and the state generated by "F U' F2 D' B U R' F' L D' R' U' L
     *                 U B' D2 R' F U2 D2" can be represented as <br>
     * 
     *                 <pre>
     ...
     * 
     *                 You can also use
     *                 {@link cs.min2phase.Tools#fromScramble(java.lang.String s)}
     *                 to convert the scramble string to the
     *                 cube definition string.
     *
     * @param maxDepth
     *                 defines the maximal allowed maneuver length. For random
     *                 cubes, a maxDepth of 21 usually will return a
     *                 solution in less than 0.02 seconds on average. With a
     *                 maxDepth of 20 it takes about 0.1 seconds on average to find
     *                 a
     *                 solution, but it may take much longer for specific cubes.
     *
     * @param probeMax
     *                 defines the maximum number of the probes of phase 2. If it
     *                 does not return with a solution, it returns with
     *                 an error code.
     *
     * @param probeMin
     *                 defines the minimum number of the probes of phase 2. So, if a
     *                 solution is found within given probes, the
     *                 computing will continue to find shorter solution(s). Btw, if
     *                 probeMin > probeMax, probeMin will be set to probeMax.
     *
     * @param verbose
     *                 determins the format of the solution(s). see USE_SEPARATOR,
     *                 INVERSE_SOLUTION, APPEND_LENGTH, OPTIMAL_SOLUTION
     *
     ...
     *         Error 6: Parity error: Two corners or two edges have to be
     *         exchanged<br>
        ...
        for (int axis = 0; axis < 18; axis += 3) {
            ...
            for (int power = 0; power < 3; power++) {
                ...
                int prun_ud = Math.max(nodeUD[maxl].doMovePrun(ud, m, false),
                        nodeUD[maxl].doMovePrunConj(ud, m));
                if (prun_ud > maxl) {
                    break;
                ...
                int prun_fb = Math.max(nodeFB[maxl].doMovePrun(fb, m, false),
                        nodeFB[maxl].doMovePrunConj(fb, m));
    ...
    // -1: no solution found
    // X: solution with X moves shorter than expectation. Hence, the length of the
    // solution is depth - X
